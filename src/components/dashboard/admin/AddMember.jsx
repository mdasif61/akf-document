  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://akf-document-server.vercel.app/api/member/signup",
        userInfo,
        config
      );
    ...
    <SideBar>
      <div className="bg-white flex items-center justify-center flex-col min-h-screen w-full">
        ...
        >
          ...
          <div className="md:w-3/4 w-full h-12 mx-auto">
            <label htmlFor="email">
              <span className="font-semibold text-black">Member Email</span>
            </label>
            <br />
            <input
              className="px-4 w-full h-full focus:outline-none border bg-white focus:bg-gray-200 text-black font-semibold"
          ...
          <div className="md:w-3/4 w-full h-12 mx-auto">
            <label htmlFor="email">
              <span className="font-semibold text-black">Member Password</span>
            </label>
            <br />
            <input
              className="px-4 w-full h-full focus:outline-none border bg-white focus:bg-gray-200 text-black font-semibold"
          ...
          <div className="md:w-3/4 w-full h-12 mx-auto">
            <label htmlFor="email">
              <span className="font-semibold text-black">Member Photo</span>
            </label>
            <br />
            <input
              onChange={(e) => photoDetails(e.target.files[0])}
              type="file"
              className="file-input file-input-bordered w-full focus:outline-none rounded-none bg-transparent"
            />
          </div>
          <button
            disabled={loading}
